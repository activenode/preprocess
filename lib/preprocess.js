  ...
  if (delim[type].useElement && context.MODULE_ELEMENTS_BASE) {
    ...
    rv = rv.replace(getRegex(type,'useElement'),function(match,line,file,include){        
        ...
        if(includedSource) {
            ...
            var extraContent = '';
            if (typeof options.inclusionProcessor == 'function') {
                extraContent = options.inclusionProcessor(elementsFilePath, process.cwd(), includedSource);
            }
            
            includedSource = line + includedSource;
            if (extraContent) {
                includedSource = line + extraContent + "\n" + includedSource;
            }
            
            for (var strKey in jsonExtraContext) {
                var val = jsonExtraContext[strKey];
                includedSource = includedSource.replace('{{'+strKey+'}}', val);
            }
